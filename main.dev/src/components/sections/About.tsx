...
const About: React.FC<AboutProps> = ({ onScrollDown }) => {
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col justify-center min-h-[85vh] py-6 px-4 md:px-8">
      ...
      <div className="mb-6">
        <h1 className="text-3xl sm:text-5xl md:text-6xl font-mono font-extrabold text-white tracking-tight mb-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 bg-[#111317]/95 border border-white/10 rounded-2xl p-5 md:p-7 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
        ...
        <div className="lg:col-span-5 flex flex-col justify-between gap-3 font-mono text-xs sm:text-sm">
          ...
          <div className="space-y-2.5">
            <div className="p-2.5 rounded-lg bg-white/[0.03] border border-white/5 hover:border-green-500/40 hover:bg-white/[0.06] transition-all">
              <div className="text-green-400 font-bold text-xs">01. STREAMING & EXTENSIONS</div>
              <div className="text-gray-400 text-[11px] sm:text-xs">Chrome MV3, ReadableStream & Service Workers</div>
            </div>

            <div className="p-2.5 rounded-lg bg-white/[0.03] border border-white/5 hover:border-green-500/40 hover:bg-white/[0.06] transition-all">
              <div className="text-cyan-400 font-bold text-xs">02. AI CRM & REALTIME SYNC</div>
              <div className="text-gray-400 text-[11px] sm:text-xs">NLP Intent Recognition & Omnichannel WebSockets</div>
            </div>

            <div className="p-2.5 rounded-lg bg-white/[0.03] border border-white/5 hover:border-green-500/40 hover:bg-white/[0.06] transition-all">
              <div className="text-emerald-400 font-bold text-xs">03. HIGH-LOAD WEB APPS</div>
              <div className="text-gray-400 text-[11px] sm:text-xs">Next.js Middleware, Custom CMS & Caching</div>
            </div>

            <div className="p-2.5 rounded-lg bg-white/[0.03] border border-white/5 hover:border-green-500/40 hover:bg-white/[0.06] transition-all">
              <div className="text-yellow-400 font-bold text-xs">04. MIGRATIONS & CLOUD</div>
              <div className="text-gray-400 text-[11px] sm:text-xs">Vue to React, Go Microservices & GCP</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {onScrollDown && (
        <button
          ...
          className="mt-6 self-center flex items-center gap-2 text-xs font-mono text-gray-500 hover:text-green-400 transition-colors animate-bounce cursor-pointer group"
        >
